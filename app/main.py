...
from config import DB_PATH, DEFAULT_MODEL_ARTIFACT, LOG_FORMAT  # noqa: E402
...
def render_predict_tab(predictor, repository) -> None:
    col_map, col_panel = st.columns([1.65, 1], gap="large")
    ...
    with col_panel:
        ...
        panel = render_trip_panel()

        if predictor is None:
            st.warning(
                "No trained model yet. Drop a .joblib file into Model/artifacts/ "
                "to enable fare predictions."
            )
        ...
        if get_estimate and ready and predictor is not None:
            trip = TripRequest(
                ...
                passenger_count=panel["passenger_count"],
                pickup_datetime=panel["pickup_datetime"],
            )
            features = TripFeatureBuilder().build(trip)
            prediction = predictor.predict(features)
            is_rush_hour = bool(features["is_rush_hour"].iloc[0])
            ride_type = panel["ride_type"]
            ...
            st.error("Cannot estimate fare -- no model artifact available.")
...
def main() -> None:
    bootstrap()
    st.set_page_config(page_title="FareCast", layout="wide", initial_sidebar_state="collapsed")
